...
class TestProcess_HighContentImaging_screen():
    ...
    def test_run(self):
        output_dir = os.path.join(ROOT_DIR,'sample_data/test/output/')
    ...
    def test_save_csv_file(self):
        new_output_path = os.path.join(ROOT_DIR,'sample_data/test/sample_scalefex_vector/Dataset01_Plate1_scalefex_new.csv')
        if os.path.exists(new_output_path):
            os.remove(new_output_path)
        test_output_path = os.path.join(ROOT_DIR,'sample_data/test/sample_scalefex_vector/Dataset01_Plate1_scalefex.csv')
    ...
    def test_segment_crop_images(self):
        ...
        np_images = np.load(os.path.join(ROOT_DIR,'sample_data/test/preprocessed_images.npy'),allow_pickle=True)
