...
class Process_HighContentImaging_screen_on_AWS:
    ...
    def __init__(self, yaml_path='parameters.yaml'):
        ...
        pd.DataFrame(columns=['plate','well','site','subset','file_path',
                    'cell_count','fail_count','computed_ids','crop_issue_ids','size_inconsistency','other_ids']).to_csv(self.fields_computed_file,index=False)
    ...
    def compute_vector(self, well):
        ...
        for site in sites:
            ...
            if np_images is not None:
                    ...
                    if crop.shape != (len(self.parameters['channel']), self.parameters['ROI'] * 2, self.parameters['ROI'] * 2, 1):
                        ...
                        try:
                            ...
                            if "DataFrame constructor not properly called!" in str(e):
                                print("An error occurred during ScaleFEx computation: DataFrame constructor not properly called! - size inconsistency.")
                                is_computed[index] = -2
                            else:
                ...
                crop_issue_ids = tuple(center_of_mass[np.argwhere(is_computed == 0).flatten(), 2])
                other_ids = tuple(center_of_mass[np.argwhere(is_computed == -1).flatten(), 2])
                size_inconsistency = tuple(center_of_mass[np.argwhere(is_computed == -2).flatten(), 2])
                ...
                compute_vec = [[self.plate, well, site, self.parameters['subset_index'], file_path,
                                len(center_of_mass), len(crop_issue_ids) + len(other_ids) + len(size_inconsistency), str(computed_ids), str(crop_issue_ids), str(size_inconsistency), str(other_ids)]]
